...
<div class="container">
    <?php if ($isDoctor) : ?>
        ...
        <form method="POST" action="">
            <label for="social_security_number">Numéro de sécurité sociale du patient :</label>
        ...
        <?php
        // Traitement de l'ajout du patient
        if (isset($_POST['add_patient'])) {
            $social_security_number = $_POST['social_security_number'];

            // Appeler la fonction pour ajouter le patient
            $result = addPatientBySocialSecurity($doctor_id, $social_security_number);

            // Vérifier le résultat et afficher le message approprié
            if ($result === true) {
